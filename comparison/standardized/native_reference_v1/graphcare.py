"""Opt-in numeric GraphCare. Upstream BAT/readouts unchanged; old source untouched.

Numeric state is added at each patient hub before the first BAT layer. Never
mutates the shared categorical embedding table. No new direct-EHR numeric bypass.
"""
import random
import torch
...
class NumericGraphCare(GraphCare):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.numeric_encoder = nn.Linear(132, self.lin.out_features, bias=False)

    def forward(self, node_ids, rel_ids, edge_index, batch, visit_node, ehr_nodes, numeric, store_attn=False, in_drop=False):
        ...
        x = self.lin(x) + self.numeric_encoder(numeric)
