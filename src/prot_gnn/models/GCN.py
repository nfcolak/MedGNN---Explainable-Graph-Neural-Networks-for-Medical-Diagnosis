    ...
    def forward(self, data, protgnn_plus=False, similarity=None):
        ...
        for i in range(self.num_gnn_layers):
            if edge_weight is not None:
                x = self.gnn_layers[i](x, edge_index, edge_weight.to(x.device))
            elif not self.gnn_layers[i].normalize:
                unit_edge_weight = torch.ones(edge_index.shape[1]).to(self.device)
                x = self.gnn_layers[i](x, edge_index, unit_edge_weight)
            else:
...
if __name__ == "__main__":
    from configs.config import model_args
