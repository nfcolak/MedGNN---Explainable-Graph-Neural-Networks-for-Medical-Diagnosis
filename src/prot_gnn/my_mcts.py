...
from configs.config import mcts_args
from torch_geometric.data import Data, Batch
from torch_geometric.utils import to_networkx
from functools import partial
from collections import Counter
from multiprocessing import Pool
from prot_gnn.utils import PlotUtils
...
if __name__ == '__main__':
    from prot_gnn.models import GnnNets, GnnNets_NC
    from prot_gnn.load_dataset import get_dataset, get_dataloader
    from configs.config import data_args, train_args, model_args
    import numpy as np
    ...
    checkpoint = torch.load(os.path.join(model_args.checkpoint, 'bbbp', 'gcn_best.pth'))
    gnnNets.update_state_dict(checkpoint['net'])
    gnnNets.to_device()
    gnnNets.eval()

    save_dir = os.path.join(os.path.dirname(model_args.checkpoint), 'mcts',
