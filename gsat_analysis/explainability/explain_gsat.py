...
records, in the schema summarize_graphxai.py / summarize_gsat.py read:
  ...
  - "BuiltinAttention" : GSAT's native deterministic per-node stochastic-
    attention probability p_v, audited with the shared fidelity metrics.
...
(shared/lib/fidelity.py). The benchmark currently evaluates node-level masks;
edge-level fidelity is outside this integration task.
...
Run (inside the ProtGNN venv, with GraphXAI on the path):
    PYTHONPATH=.:external/GraphXAI-main .venv-protgnn/bin/python3 \
        ...
        --ckpt comparison/standardized/results/gsat/star/1234/gsat_model.pt \
        --graph_structure star \
        --canonical_split comparison/canonical_split.json \
        --max_graphs 50 --out_dir comparison/standardized/results/gsat/star/1234/explanations
"""
import argparse
import inspect
import json
...
def _record_importance(
    rec_expl, name, imp, wrapper, x, edge_index, target_class,
    target_provenance, batch, top_k, agg,
):
    order = list(np.argsort(np.abs(imp))[::-1][:top_k])
    fp = fidelity_plus(wrapper, x, edge_index, imp, target_class, batch)
    fm = fidelity_minus(wrapper, x, edge_index, imp, target_class, batch)
    rec_expl[name] = {
        "target": {
    ...
    for gi, batch in enumerate(test_loader):
        ...
        rec = {
            ...
            "explanation_target": {
                ...
                "class_id": pred,
                "class_name": id_to_class.get(pred, str(pred)),
            },
        ...
        try:
            ...
            _record_importance(rec["explanations"], "BuiltinAttention", imp,
                               wrapper, x, edge_index, pred, "model_prediction",
                               null_batch, args.top_nodes, agg)
        except Exception as e:
...
def select_standardized_records(records, requested_subject_ids):
    """Select all cohort graphs by subject ID and fail on any missing ID."""
    return select_exact_subject_records(records, requested_subject_ids)


def explain_gsat_record(gsat, graph, *, topology, seed, graphxai=False):
    """Explain one GSAT graph using its deterministic native node scores."""
    device = torch.device("cpu")
    graph = graph.to(device)
    x = graph.x
    edge_index = graph.edge_index
    batch = torch.zeros(x.size(0), dtype=torch.long, device=device)
    wrapper = GSATGraphXAIWrapper(gsat).to(device).eval()
    wrapper.set_context(x, edge_index, batch)
    faithful, difference = wrapper.verify(x, edge_index, batch)
    if not faithful:
        raise RuntimeError(
            f"GSAT wrapper is not faithful; max_abs_diff={difference:.3e}."
        )
    importance = wrapper.builtin_node_importance(x, edge_index, batch)
    ...
    record = {
        ...
        "attribution_method": "gsat_builtin_node_attention_probability",
...
def main(*, checkpoint, graph_structure, canonical_split, cohort, dataset, seed, out_dir, cohort_size=COHORT_SIZE):
    """Run the mandatory fixed-cohort standardized GSAT explanation path."""
    ...
    if not os.path.isfile(checkpoint):
        raise FileNotFoundError(f"GSAT checkpoint missing: {checkpoint}")
    set_seed(spec.seed)
    device = torch.device("cpu")
    gsat, checkpoint_structure = load_model(checkpoint, device)
    if checkpoint_structure != spec.structure:
        raise ValueError("GSAT checkpoint topology does not match request.")
    ...
    loaders = get_dataloader(
        dataset_object,
        batch_size=1,
        data_split_ratio=list(cfg.split_ratio),
        seed=spec.seed,
        canonical_split=canonical_split,
    )
    ...
    records = [
        explain_gsat_record(gsat, graph, topology=spec.structure, seed=spec.seed, graphxai=True)
...
def cli(argv=None):
    ...
    standardized_flags = {"--cohort", "--dataset", "--seed", "--checkpoint"}
    canonical_requested = any(
        flag in arguments for flag in ("--canonical_split", "--canonical-split")
    )
    if not canonical_requested and not any(flag in arguments for flag in standardized_flags):
        return legacy_main(arguments)
    parser = argparse.ArgumentParser(description="Fixed-cohort standardized GSAT explanations")
    parser.add_argument("--checkpoint", "--ckpt", dest="checkpoint", required=True)
    parser.add_argument("--graph-structure", "--graph_structure", "--graph", dest="graph_structure", required=True)
    parser.add_argument("--canonical-split", "--canonical_split", dest="canonical_split", required=True)
    ...
    return main(**vars(parser.parse_args(arguments)))
