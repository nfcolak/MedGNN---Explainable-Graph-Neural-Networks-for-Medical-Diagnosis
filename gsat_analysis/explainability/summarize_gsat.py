...
Usage:
    PYTHONPATH=. python3 gsat_analysis/explainability/summarize_gsat.py \
        --explanations_dir comparison/standardized/results/gsat/star/1234/explanations
"""
import argparse
import csv
import glob
...
def build_rows(files, top_nodes=4):
    ...
    for fp_path in files:
        ...
        target = d.get("explanation_target") or {}
        row = {
            ...
            "target_class": target.get("class_name", target.get("class_id", "")),
        }
        top_idx_per_expl = {}
        for e in EXPLAINERS:
            s = SHORT[e]
...
def aggregate_row(rows, agg, agg_fp, agg_fm, n_agree, n_agree_total):
    summary = {
        ...
        "num_nodes": "", "target_provenance": "", "target_class_id": "",
        "target_class": "",
    }
    for e in EXPLAINERS:
        s = SHORT[e]
...
def header_order():
    h = [
        "graph", "subject_id", "prediction", "actual", "result", "num_nodes",
        "target_provenance", "target_class_id", "target_class",
    ]
    for e in EXPLAINERS:
        s = SHORT[e]
