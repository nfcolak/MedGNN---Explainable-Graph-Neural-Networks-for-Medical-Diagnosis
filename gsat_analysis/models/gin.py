...
Sized to match ProtGNN's capacity (hidden 128 x 3 layers). ``mean`` readout
matches the benchmark's other methods, while the paper uses add-pool; reports
must identify this model-native deviation.

Contract follows the official Graph-COM/GSAT GIN interface:
...
class GINConv(_BaseGINConv):
    ...
    ``x_j * edge_atten`` before aggregation, following the official GSAT
    convolution pattern.
    """
...
class SingletonSafeBatchNorm1d(nn.BatchNorm1d):
    """Use running statistics for a one-node training batch (variance undefined).

    Preserve standard BatchNorm exactly for every batch with at least two nodes.
    The singleton branch neither updates running statistics nor their counter;
    affine parameters and upstream embeddings still receive gradients.
    """
...
class GIN(nn.Module):
    ...
    def _mlp(in_dim, out_dim):
        return nn.Sequential(
            nn.Linear(in_dim, out_dim), SingletonSafeBatchNorm1d(out_dim), nn.ReLU(),
