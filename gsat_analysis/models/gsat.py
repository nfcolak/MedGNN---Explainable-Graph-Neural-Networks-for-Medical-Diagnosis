...
Reimplemented from the paper and the official Graph-COM/GSAT source against
this project's per-patient PyG graphs. The extractor restores the official
graph-aware InstanceNorm behavior.
...
noise, at eval) is GSAT's native explanation. Higher p_v means the node is more
likely to be kept; shared fidelity metrics audit the resulting explanation.
"""
import torch
...
class BatchSequential(nn.Sequential):
    """Sequential container that forwards graph membership to InstanceNorm."""
...
class ExtractorMLP(nn.Module):
    ...
    Uses the official GSAT graph-aware InstanceNorm MLP pattern."""
    ...
    def __init__(self, hidden_dim, attention_level="node", dropout=0.5):
        ...
        self.mlp = BatchSequential(
            nn.Linear(in_dim, hidden_dim * 2), InstanceNorm(hidden_dim * 2),
            nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(hidden_dim * 2, hidden_dim), InstanceNorm(hidden_dim),
            nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, emb, edge_index, batch):
        if self.attention_level == "edge":
            src, dst = edge_index
            return self.mlp(torch.cat([emb[src], emb[dst]], dim=-1), batch[src])
        return self.mlp(emb, batch)
...
class GSAT(nn.Module):
    ...
    def predict(self, x, edge_index, batch):
        ...
        p = self.extractor(emb, edge_index, batch).sigmoid()
        if self.attention_level == "edge":
    ...
    def node_importance(self, data):
        ...
        self.eval()
        emb = self.clf.get_emb(data.x, data.edge_index, batch=data.batch)
        logits = self.extractor(emb, data.edge_index, data.batch).squeeze(-1)
        p = logits.sigmoid()
